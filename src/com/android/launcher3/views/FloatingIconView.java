...
public class FloatingIconView extends View implements
    ...
    private void getIcon(View v, ItemInfo info, boolean isOpening,
        ...
        boolean supportsAdaptiveIcons = Build.VERSION.SDK_INT >= Build.VERSION_CODES.O;
    ...
    private int getOffsetForIconBounds(Drawable drawable) {
        ...
        try (LauncherIcons li = LauncherIcons.obtain(getContext())) {
            Utilities.scaleRectAboutCenter(bounds, li.getNormalizer().getScale(drawable, null, new Path(), new boolean[1]));
        }
    ...
    private AnimatorSet createFadeAnimation(View originalView, DragLayer dragLayer) {
        ...
        if (originalView instanceof BubbleTextView) {
            ...
            btv.forceHideBadge(true);
            fade.addListener(new AnimatorListenerAdapter() {
                @Override
                public void onAnimationEnd(Animator animation) {
                    btv.forceHideBadge(false);
                }
            });
        }
        ...
        if (originalView instanceof FolderIcon) {
            ...
            fade.addListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    if (folderIcon.hasBadge()) {
                        folderIcon.createBadgeScaleAnimator(0f, 1f).start();
                    }
                }
            });
        } else {

...
class BaiduSearchProvider(context: Context) : SearchProvider(context) {
    ...
    override fun getIcon(colored: Boolean): Drawable = context.getDrawable(R.drawable.ic_baidu).mutate().apply {
    ...
    override fun getVoiceIcon(colored: Boolean): Drawable = context.getDrawable(R.drawable.ic_mic_color).mutate().apply {
         setTint(if(colored) Color.parseColor("#2d03e4") else Color.WHITE)
    }
}
...
class DoubleShadowIconDrawable(icon: Drawable, context: Context) : LayerDrawable(emptyArray()) {
    ...
    private fun generateShadowDrawable(iconBitmap: Bitmap, context: Context): Drawable {
        ...
        val ambientShadowRadius =
            res.getDimensionPixelSize(R.dimen.ambient_text_shadow_radius).toFloat()
        ...
        if (keyShadowRadius != 0f) {
            ...
            canvas.drawBitmap(
                alphaBitmap,
                alphaOffset[0].toFloat() + keyShadowDx,
                alphaOffset[1].toFloat() + keyShadowDy,
                alphaPaint,
            )
        }

...
class LawnchairShortcut(private val context: Context) {
    ...
    inner class ShortcutEntry(key: String, val shortcut: SystemShortcut.Factory<in Launcher>,
                              enabled: Boolean) {
    ...
    class Uninstall(private val target: BaseDraggingActivity, private val itemInfo: ItemInfo,
        ...
        override fun onClick(v: View?) {
            ...
            val i = Intent.parseUri(target.getString(R.string.delete_package_intent), 0)
                    .setData(Uri.fromParts("package", componentName.packageName,
        ...
        companion object {
            ...
            val FACTORY = Factory<BaseDraggingActivity> { launcher, itemInfo ->
                if (itemInfo is ItemInfoWithIcon && itemInfo.runtimeStatusFlags.hasFlag(
                                FLAG_SYSTEM_YES)) {
                    null
                } else {
            ...
            private fun getUninstallTarget(target: BaseDraggingActivity,
                                           item: ItemInfo): ComponentName? {
                if (item.itemType == ITEM_TYPE_APPLICATION && item.id == ItemInfo.NO_ID) {
                    val intent = item.intent
                    val user = item.user
                    if (intent != null) {
                        val info = target.getSystemService(LauncherApps::class.java)
    ...
    class Remove(private val target: Launcher, private val itemInfo: ItemInfo) :
            SystemShortcut<Launcher>(R.drawable.ic_remove_no_shadow,
                                     R.string.remove_drop_target_label, target,
                                     itemInfo) {
        override fun onClick(v: View?) {
            AbstractFloatingView.closeAllOpenViews(target)
        ...
        companion object {
            val FACTORY = Factory<Launcher> { target, itemInfo ->
                when {
                    ...
                    itemInfo is WorkspaceItemInfo || itemInfo is LauncherAppWidgetInfo || itemInfo is FolderInfo -> Remove(
                            target, itemInfo)
                    else -> null
                }
            }
    ...
    class SesameSettings(private val target: Launcher, itemInfo: ItemInfo,
                         private val settingsIntent: Intent) :
            SystemShortcut<Launcher>(R.drawable.ic_sesame, R.string.shortcut_sesame, target,
                                     itemInfo) {
        override fun onClick(v: View?) {
        ...
        companion object {
            val FACTORY = object : Factory<Launcher> {
                override fun getShortcut(launcher: Launcher,
                                         itemInfo: ItemInfo): SystemShortcut<Launcher>? {
                    if (itemInfo.itemType != ITEM_TYPE_APPLICATION) return null
                    val packageName =
                            itemInfo.targetComponent?.packageName ?: itemInfo.intent.`package`

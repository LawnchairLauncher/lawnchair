...
class BundleDiff(first: Bundle, second: Bundle) : Diff<String, Bundle>(first, second) {
    ...
    override val removed: List<String> by lazy {
        first.keySet().filterNot { second.containsKey(it) }
    }
    override val changed: List<String> by lazy {
        first.keySet().filter { second.containsKey(it) }.filter { first[it] != second[it] }
    }
}

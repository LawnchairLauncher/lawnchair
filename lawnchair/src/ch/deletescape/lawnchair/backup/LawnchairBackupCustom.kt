...
class LawnchairBackupCustom(val context: Context, val path: String) {
    ...
    private inline fun readZip(body: (ZipInputStream) -> Unit) {
        try {
            val inStream = FileInputStream(File(path))
            val zipIs = ZipInputStream(inStream)
            try {
                body(zipIs)
            } catch (t: Throwable) {
                Log.e(TAG, "Unable to read zip for $path", t)
            } finally {
                zipIs.close()
                inStream.close()
            }
    ...
    fun restore(contents: Int): Boolean {
        try {
            ...
            try {
                ...
                Log.e(TAG, "Failed to restore $path", t)
            } finally {
                zipIs.close()
                inStream.close()
                return success
    ...
    class MetaLoader(val backup: LawnchairBackupCustom) {
    ...
    companion object {
        ...
        /*fun listLocalBackups(context: Context): List<LawnchairBackupCustom> {
            return getFolder().listFiles { file -> file.extension == EXTENSION }
                    ...
                    ?.map { LawnchairBackupCustom(context, it) }

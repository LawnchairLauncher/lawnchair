...
interface AccuWeatherRetrofitService {
    ...
    fun getHourly(@Path("key") key: String,
                  @Query("language") language: String): Call<List<AccuHourlyForecastGSon>>

    @GET("localweather/v1/{key}")
    fun getLocalWeather(@Path("key") key: String,
                        @Query("language") language: String): Call<AccuLocalWeatherGSon>
}

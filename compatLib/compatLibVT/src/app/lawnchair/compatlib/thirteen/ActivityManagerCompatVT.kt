...
open class ActivityManagerCompatVT : ActivityManagerCompatVS() {
    ...
    override fun startRecentsActivity(intent: Intent, eventTime: Long, runnerCompat: RecentsAnimationRunnerCompat) {
        val runner: IRecentsAnimationRunner? = if (runnerCompat != null) {
    ...
    override fun getTaskSnapshot(taskId: Int, isLowResolution: Boolean, takeSnapshotIfNeeded: Boolean): TaskSnapshot? {
        try {
            // android13 qpr1
            return ActivityTaskManager.getService().getTaskSnapshot(taskId, isLowResolution, true)
        } catch (e: RemoteException) {

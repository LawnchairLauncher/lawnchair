...
class ProxyImpl(val context: Context) : ILauncherClientProxy.Stub() {
    ...
    override fun init(callback: ILauncherClientProxyCallback): Int {
        allowed = "ch.deletescape.lawnchair" == getCallingPackage()
        enforcePermission()
    ...
    private fun getCallingPackage() = context.packageManager.getNameForUid(Binder.getCallingUid())

    fun enforcePermission() {
        if (!allowed)
            throw SecurityException("${getCallingPackage()} is not allowed to call this service")
    }

...
import app.mica.preferences.not
import app.mica.preferences.preferenceManager
import app.mica.preferences2.PreferenceManager2
import app.mica.preferences2.preferenceManager2
import app.mica.search.algorithms.MicaSearchAlgorithm
...
import app.mica.ui.preferences.components.HiddenAppsInSearchPreference
...
import app.mica.ui.preferences.components.controls.MainSwitchPreference
import app.mica.ui.preferences.components.controls.SwitchPreference
import app.mica.ui.preferences.components.controls.TwoTargetSwitchPreference
import app.mica.ui.preferences.components.layout.PreferenceGroup
import app.mica.ui.preferences.navigation.SearchProviderPreference
...
fun DrawerSearchPreference(
    ...
    MainSwitchPreference(
        ...
        PreferenceGroup(heading = stringResource(id = R.string.show_search_result_types)) {
            if (searchAlgorithm != MicaSearchAlgorithm.ASI_SEARCH) {
                val canDisable = searchAlgorithm != MicaSearchAlgorithm.APP_SEARCH
            ...
            when (searchAlgorithm) {
                MicaSearchAlgorithm.LOCAL_SEARCH -> {
                    LocalSearchSettings(
                        prefs = prefs,
                        prefs2 = prefs2,
                        context = context,
                ...
                MicaSearchAlgorithm.ASI_SEARCH -> {
...
private fun SearchProvider(
    ...
    val searchAlgorithmEntries = remember {
        sequenceOf(
            ListPreferenceEntry(MicaSearchAlgorithm.APP_SEARCH) { stringResource(R.string.search_algorithm_app_search) },
            ListPreferenceEntry(MicaSearchAlgorithm.LOCAL_SEARCH) { stringResource(R.string.search_algorithm_global_search_on_device) },
            ListPreferenceEntry(MicaSearchAlgorithm.ASI_SEARCH) { stringResource(R.string.search_algorithm_global_search_via_asi) },
        ).filter {
            when (it.value) {
                MicaSearchAlgorithm.ASI_SEARCH -> MicaSearchAlgorithm.isASISearchEnabled(
                    context,
                )

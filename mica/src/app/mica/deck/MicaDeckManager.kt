...
import app.mica.launcherNullable
...
class MicaDeckManager(private val context: Context) {

    // TODO

    private val launcher = context.launcherNullable ?: MicaLauncher.instance?.launcher

    suspend fun enableMicaDeck(
        onProgress: ((String) -> Unit)? = null,
    ) = withContext(Dispatchers.IO) {
        val completionDeferred = CompletableDeferred<Unit>()

        if (!backupExists("bk")) createBackup("bk")
        if (backupExists("micadeck")) {
            onProgress?.invoke("Restoring previous layout...")
            restoreBackup("micadeck")
    ...
    suspend fun disableMicaDeck() = withContext(Dispatchers.IO) {
        if (backupExists("bk")) {
            createBackup("micadeck")
            restoreBackup("bk")
        }
    }
    ...
    }.onFailure { Log.e("MicaDeckManager", "Failed to create backup: $suffix", it) }
    ...
    private fun restoreBackup(suffix: String) = runCatching {
        ...
        postRestoreActions()
    }.onFailure { Log.e("MicaDeckManager", "Failed to restore backup: $suffix", it) }

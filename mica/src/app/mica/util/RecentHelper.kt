...
object RecentHelper {
    ...
    fun clearAllTaskStacks(context: Context) {
        try {
            ...
            val recentsView = launcher.getOverviewPanel<RecentsView<MicaLauncher, *>>()
    ...
    fun isAppLocked(packageName: String, context: Context): Boolean {
        val pref = context.getSharedPreferences(
            MicaLockedStateController.TASK_LOCK_STATE,
            Context.MODE_PRIVATE,
        )

        val lockedApps = pref.getStringSet(
            MicaLockedStateController.TASK_LOCK_LIST_KEY_WITH_USERID,
            emptySet(),
        )

...
import app.mica.allapps.views.SearchItemBackground
import app.mica.allapps.views.SearchResultView.Companion.EXTRA_QUICK_LAUNCH
import app.mica.preferences2.PreferenceManager2
import app.mica.preferences2.firstCached
import app.mica.search.MicaSearchAdapterProvider
import app.mica.search.adapter.START_PAGE
import app.mica.search.adapter.SearchAdapterItem
import app.mica.search.adapter.SearchTargetCompat
import app.mica.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_APPLICATION
import app.mica.search.adapter.SearchTargetCompat.Companion.RESULT_TYPE_SHORTCUT
...
sealed class MicaSearchAlgorithm(
    ...
    protected fun transformSearchResults(results: List<SearchTargetCompat>): List<SearchAdapterItem> {
        val filtered = results
            ...
            .filter { MicaSearchAdapterProvider.viewTypeMap[it.layoutType] != null }
...
//        layoutType == EDUCARD -> transparentBackground
    ...
    companion object {
        ...
        fun isASISearchEnabled(context: Context): Boolean {
            ...
            if (!MicaApp.isRecentsEnabled) return false

            if (!ranCompatibilityCheck) {
                ranCompatibilityCheck = true
                MicaASISearchAlgorithm.checkSearchCompatibility(context)
            }
            return true
        }

        fun create(context: Context): MicaSearchAlgorithm {
            val prefs = PreferenceManager2.getInstance(context)
            val searchAlgorithm = prefs.searchAlgorithm.firstCached()

            return when {
                searchAlgorithm == ASI_SEARCH && isASISearchEnabled(context) -> MicaASISearchAlgorithm(
                    context,
                )

                searchAlgorithm == LOCAL_SEARCH -> MicaLocalSearchAlgorithm(context)

                else -> MicaAppSearchAlgorithm(context)
            }
        }
    }
}

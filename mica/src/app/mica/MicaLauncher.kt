...
import app.mica.MicaApp.Companion.showQuickstepWarningIfNecessary
import app.mica.compat.MicaQuickstepCompat
import app.mica.data.AppDatabase
import app.mica.data.wallpaper.service.WallpaperService
import app.mica.gestures.GestureController
import app.mica.gestures.VerticalSwipeTouchController
import app.mica.gestures.config.GestureHandlerConfig
import app.mica.gestures.ui.MicaShortcutActivity
import app.mica.nexuslauncher.OverlayCallbackImpl
...
import app.mica.root.RootHelperManager
import app.mica.root.RootNotAvailableException
import app.mica.theme.ThemeProvider
import app.mica.ui.popup.LauncherOptionsPopup
import app.mica.ui.popup.MicaShortcut
import app.mica.util.getThemedIconPacksInstalled
import app.mica.util.unsafeLazy
import app.mica.views.MicaFloatingSurfaceView
...
class MicaLauncher : QuickstepLauncher() {
    ...
    private val statusBarClockListener = object : StateManager.StateListener<LauncherState> {
        override fun onStateTransitionStart(toState: LauncherState) {
            when (toState) {
                ...
                is AllAppsState,
                -> {
                    MicaApp.instance.restoreClockInStatusBar()
                }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        layoutInflater.factory2 = MicaLayoutFactory(this)
        ...
        if (prefs.autoLaunchRoot.get()) {
            lifecycleScope.launch {
                try {
                    RootHelperManager.INSTANCE.get(this@MicaLauncher)
    ...
    override fun onNewIntent(intent: Intent?) {
        if (intent != null && intent.action == MicaShortcutActivity.START_ACTION) {
            val handlerString = intent.getStringExtra(MicaShortcutActivity.EXTRA_HANDLER)
    ...
    override fun getAllAppsItemLongClickListener(): View.OnLongClickListener {
        return View.OnLongClickListener { view ->
            if (view is FolderIcon && view.mInfo.id != ItemInfo.NO_ID) {
                MicaShortcut.showAppDrawerFolderPopup(this, view)
            } else {
    ...
    override fun getSupportedShortcuts(container: Int): Stream<SystemShortcut.Factory<*>> = Stream.concat(
        ...
        Stream.concat(
            Stream.of(MicaShortcut.UNINSTALL, MicaShortcut.CUSTOMIZE, MicaShortcut.OPEN_IN_STORE),
            if (MicaApp.isRecentsEnabled) Stream.of(MicaShortcut.PAUSE_APPS) else Stream.empty(),
        ),
    )
    ...
    override fun handleGestureContract(intent: Intent) {
        if (!MicaApp.isRecentsEnabled && prefs.enableGnc.get()) {
            ...
            if (gnc != null) {
                ...
                MicaFloatingSurfaceView.show(this, gnc)
            }
        }
    }
    ...
    override fun makeDefaultActivityOptions(splashScreenStyle: Int): ActivityOptionsWrapper {
        ...
        val options = if (Utilities.ATLEAST_Q) {
            MicaQuickstepCompat.activityOptionsCompat.makeCustomAnimation(
                this,
                0,
                0,
    ...
    private fun restartIfPending() {
        when {
            sRestartFlags and FLAG_RESTART != 0 -> micaApp.restart(false)
    ...
    companion object {
        ...
        val instance get() = MicaApp.launcher
    }
}

val Context.launcher: MicaLauncher
    get() = BaseActivity.fromContext(this)

val Context.launcherNullable: MicaLauncher? get() = try {
    launcher
